...
export let history: import('history').BrowserHistory | undefined;
...
export default class Router extends Component<Props, State> {
  constructor(props: Props) {
    super(props);
    this.state = {
      path: history ? this.parse(history.location.pathname) : props.path,
    };
  }
  ...
  componentDidMount() {
    this.unlisten = history?.listen(({ location }) =>
      this.setState({
        path: this.parse(location.pathname),
      })
    );
  }
