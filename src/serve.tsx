...
export default function serve(o: Options) {
  ...
  let fallback = build404({ data, sheets: ['app.css'], favicon });
  let routes = buildPageInfo(data).reduce<Record<string, string>>(
    (acc, info) => {
  ...
  const callback = (event: 'change' | 'add' | 'delete') => (
    ...
    try {
      ...
      fallback = build404({ data, sheets: ['app.css'], favicon });
      routes = buildPageInfo(data).reduce<Record<string, string>>(
        (acc, info) => {
          acc[info.link] = buildHTML({
            data,
            info,
  ...
  if (scripts) {
    scripts.forEach((name) => {
      app.get(`/scripts/${path.basename(name)}`, (_, res) => {
        res.sendFile(name);
      });
    });
  }
